...
const useAIChatStreamHandler = () => {
  ...
  const handleStreamResponse = useCallback(
    async (input: string | FormData) => {
      ...
      // Check if this is a retry (indicated by __retry flag in formData)
      const isRetry = formData.get('__retry') === 'true'
      formData.delete('__retry') // Remove the flag before sending to API
      ...
      // Extract attachments metadata if present
      const attachmentsJson = formData.get('attachments')
      const attachments = attachmentsJson
        ? JSON.parse(attachmentsJson as string)
        : undefined

      // Only add user and agent messages if this is NOT a retry
      if (!isRetry) {
        addMessage({
          role: 'user',
      ...
      try {
        ...
        if (mode === 'team' && selectedTeamId) {
          RunUrl = APIRoutes.TeamRun(endpointUrl, selectedTeamId)
        ...
        // Only append session_id if it exists (for continuing conversations)
        // For new conversations, don't send it so AgentOS creates a new one
        if (sessionId) {
          formData.append('session_id', sessionId)
        }
        
        // Add user_id from authenticated user for session persistence
        if (user?.id) {
        ...
        await streamResponse({
          ...
          onChunk: (chunk: RunResponse) => {
            if (
              ...
              // Store the run_id for cancellation
              if (chunk.run_id) {
              ...
              // Handle cancellation separately - not an error, just stopped
              setMessages((prevMessages) => {
                const newMessages = [...prevMessages]
                const lastMessage = newMessages[newMessages.length - 1]
                if (lastMessage && lastMessage.role === 'agent') {
                  // Keep the partial content that was generated before cancellation
                  lastMessage.content = lastMessage.content || 'Run was cancelled by user.'
                }
                return newMessages
              })
